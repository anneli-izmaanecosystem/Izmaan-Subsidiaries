...
type Employee = { id: number; name: string; idNumber: string | null; grossPay: string; deductions: string; netPay: string; uifEmployee: string }
type Totals = { grossPay: string; deductions: string; netPay: string; uifEmployee: string }
type Settings = { name: string; address: string | null; coidRef: string | null }
...
// Default range: current COID year (1 March) to today, or the prior 12 months if before March.
...
export default function CoidaSummaryPage() {
  ...
  const [data, setData] = useState<{ employees: Employee[]; totals: Totals } | null>(null)
  const [settings, setSettings] = useState<Settings | null>(null)
  const [loading, setLoading] = useState(true)

  // Old data (if any) stays on screen while a new range loads — no loading flicker.
  useEffect(() => {
    Promise.all([
      ...
      .then(([summary, companySettings]) => { setData(summary); setSettings(companySettings) })
  ...
  function downloadCsv() {
    ...
    const meta = [
      [`Employer: ${settings?.name ?? ''}`],
      [`COID Reference: ${settings?.coidRef ?? 'N/A'}`],
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="print:hidden flex items-center justify-between mb-6 max-w-3xl mx-auto">
        <Link href="/dashboard/payroll" className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600">
      ...
      <div className="max-w-3xl mx-auto bg-white shadow-sm rounded-xl print:shadow-none print:rounded-none print:max-w-full">
        <div className="px-8 py-6 border-b border-gray-200">
          ...
          <div className="mt-3 flex gap-8 text-sm">
            <div>
              ...
              <p className="font-semibold text-gray-800">{settings?.coidRef ?? '—'}</p>
            </div>
            <div>
        ...
        <div className="px-8 py-4 border-t border-gray-100 bg-gray-50 text-xs text-gray-500 leading-5">
          Sums every finalised payroll run whose pay period falls within the dates above, computed live from payroll
          records. This is an earnings-summary export only — it does not apply a COID tariff rate, so cross-check
          against the actual filed COIDA/UIF submissions before relying on it for a return.
        </div>
      </div>
